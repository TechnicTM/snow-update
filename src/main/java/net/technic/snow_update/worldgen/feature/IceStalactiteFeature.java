...
public class IceStalactiteFeature extends Feature<DripstoneClusterConfiguration>{
    public IceStalactiteFeature(Codec<DripstoneClusterConfiguration> pCodec) {
        super(pCodec);
    }
    ...
    private void placeColumn(WorldGenLevel p_225016_, RandomSource p_225017_, BlockPos p_225018_, int p_225019_, int p_225020_, float p_225021_, double p_225022_, int p_225023_, float p_225024_, DripstoneClusterConfiguration p_225025_) {
        Optional<Column> optional = Column.scan(p_225016_, p_225018_, p_225025_.floorToCeilingSearchRange, IceStalactiteUtils::isEmptyOrWater, IceStalactiteUtils::isNeitherEmptyNorWater);
        if (optional.isPresent()) {
            ...
            if (optionalint.isPresent() || optionalint1.isPresent()) {
                ...
                if (optionalint.isPresent()) {
                IceStalactiteUtils.growPointedDripstone(p_225016_, p_225018_.atY(optionalint.getAsInt() - 1), Direction.DOWN, j3, flag3);
                }

                if (optionalint2.isPresent()) {
                IceStalactiteUtils.growPointedDripstone(p_225016_, p_225018_.atY(optionalint2.getAsInt() + 1), Direction.UP, j1, flag3);
                }
    ...
    private boolean canPlacePool(WorldGenLevel p_159620_, BlockPos p_159621_) {
        ...
        if (!blockstate.is(Blocks.ICE) && !blockstate.is(SnowBlockRegistry.ICE_SPIKE_BLOCK.get()) && !blockstate.is(SnowBlockRegistry.POINTED_ICE_STALACTITE.get())) {
    ...
    private void replaceBlocksWithDripstoneBlocks(WorldGenLevel p_159589_, BlockPos p_159590_, int p_159591_, Direction p_159592_) {
        ...
        for(int i = 0; i < p_159591_; ++i) {
            if (!IceStalactiteUtils.placeIceSpikeBlockIfPossible(p_159589_, blockpos$mutableblockpos)) {
                return;
            }
