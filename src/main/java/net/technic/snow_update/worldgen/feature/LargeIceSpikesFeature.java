...
import net.technic.snow_update.worldgen.util.IceStalactiteUtils;

public class LargeIceSpikesFeature extends Feature<LargeDripstoneConfiguration>{

    public LargeIceSpikesFeature(Codec<LargeDripstoneConfiguration> pCodec) {
        super(pCodec);
    }
    ...
    public boolean place(FeaturePlaceContext<LargeDripstoneConfiguration> pContext) {
        ...
        if (!IceStalactiteUtils.isEmptyOrWater(worldgenlevel, blockpos)) {
            return false;
        } else {
            Optional<Column> optional = Column.scan(worldgenlevel, blockpos, largedripstoneconfiguration.floorToCeilingSearchRange, IceStalactiteUtils::isEmptyOrWater, IceStalactiteUtils::isDripstoneBaseOrLava);
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargeDripstone largedripstonefeature$largedripstone = makeDripstone(blockpos.atY(column$range.ceiling() - 1), false, randomsource, k, largedripstoneconfiguration.stalactiteBluntness, largedripstoneconfiguration.heightScale);
                    LargeDripstone largedripstonefeature$largedripstone1 = makeDripstone(blockpos.atY(column$range.floor() + 1), true, randomsource, k, largedripstoneconfiguration.stalagmiteBluntness, largedripstoneconfiguration.heightScale);
                    ...
                    if (largedripstonefeature$largedripstone.isSuitableForWind(largedripstoneconfiguration) && largedripstonefeature$largedripstone1.isSuitableForWind(largedripstoneconfiguration)) {
                        largedripstonefeature$windoffsetter = new WindOffsetter(blockpos.getY(), randomsource, largedripstoneconfiguration.windSpeed);
                    } else {
    ...
    private void placeDebugMarkers(WorldGenLevel p_159962_, BlockPos p_159963_, Column.Range p_159964_, WindOffsetter p_159965_) {
        ...
        for(BlockPos.MutableBlockPos blockpos$mutableblockpos = p_159963_.atY(p_159964_.floor() + 2).mutable(); blockpos$mutableblockpos.getY() < p_159964_.ceiling() - 1; blockpos$mutableblockpos.move(Direction.UP)) {
            ...
            if (IceStalactiteUtils.isEmptyOrWater(p_159962_, blockpos) || p_159962_.getBlockState(blockpos).is(SnowBlockRegistry.ICE_SPIKE_BLOCK.get())) {
    ...
    static final class LargeDripstone {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel p_159990_, WindOffsetter p_159991_) {
            while(this.radius > 1) {
                ...
                for(int j = 0; j < i; ++j) {
                    ...
                    if (IceStalactiteUtils.isCircleMostlyEmbeddedInStone(p_159990_, p_159991_.offset(blockpos$mutableblockpos), this.radius)) {
        ...
        private int getHeightAtRadius(float p_159988_) {
            return (int) IceStalactiteUtils.getDripstoneHeight((double)p_159988_, (double)this.radius, this.scale, this.bluntness);
        }
                    ...
                    if (!(f > (float)this.radius)) {
                        ...
                        if (k > 0) {
                            ...
                            for(int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; ++i1) {
                                ...
                                if (IceStalactiteUtils.isEmptyOrWaterOrLava(p_225146_, blockpos)) {
                                    flag = true;
                                    Block block = SnowBlockRegistry.ICE_SPIKE_BLOCK.get();
